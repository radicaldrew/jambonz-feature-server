...
class CallSession extends Emitter {
   ...
   */
  constructor({logger, application, srf, tasks, callInfo}, ghostSession = false) {
    super();
    this.logger = logger;
    ...
    if (!ghostSession) sessionTracker.add(this.callSid, this);
  }
 ...
 */
  ...
  */
  ...
  kill() {
    this.logger.debug('CallSession:kill');
    if (this.currentTask) {
      this.currentTask.kill();
      this.currentTask = null;
    }
        ...
        .then((req) => {
          req.on('response', (res) => {
            if ([200, 202].includes(res.status)) {
              ...
              /**
               * we can hang up (discarding remaining tasks)
               * because call is being moved to a new server
               */
              this._callerHungup();
            }
            else resolve(false);
  ...
  getRemainingTaskData() {
    ...
    for (const task of this.tasks) {
      found = found || task === this.currentTask;
      if (found) {
        const o = {};
        o[task.name] = task.getJSON();
